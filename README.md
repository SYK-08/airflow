...
<!-- START doctoc generated TOC please keep commenting here to allow auto-update -->
...
it is not a high priority. You should only use Linux-based distros as a "Production" execution environment
...
Documentation for dependent projects like provider packages, Docker image, and Helm Chart, you'll find in [the documentation index](https://airflow.apache.org/docs/).

## Installing from PyPI

We publish Apache Airflow as `apache-airflow` package in PyPI. Installing it, however, might be sometimes tricky
because Airflow is a bit of both a library and an application. Libraries usually keep their dependencies open, and
applications usually pin them, but we should do neither or both simultaneously. We decided to keep
...
- [PyPI releases](https://pypi.org/project/apache-airflow/) to install Airflow using the standard `pip` tool
...
There are a few specific rules that we agreed to that define details of versioning of the different
packages:
...
* **Airflow Providers**: SemVer rules apply to changes in the particular provider's code only.
  ...
  is a breaking change for the provider because installing the new provider might automatically
  upgrade Airflow (which might be an undesired side effect of upgrading the provider).
...
1. We drop support for Python and Kubernetes versions when they reach EOL. Except for Kubernetes, a
   ...
   support for those EOL versions in the main right after the EOL date, and it is effectively removed when we release
...
2. We support a new version of Python/Kubernetes in the main after they are officially released, as soon as we
...
* Base Python installation in versions supported at the time of release for the MINOR version of
  Airflow released (so there could be different versions for 2.3 and 2.2 lines for example)
...
* In the future Airflow might also support a "slim" version without providers or database clients installed

The version of the base OS image is the stable version of Debian. Airflow supports using all currently active
stable versions - as soon as all Airflow dependencies support building, we set up the CI pipeline for
...
the switch happened. In the case of the Bullseye switch - 2.3.0 version used ``Debian Bullseye``.
...
Support for ``Debian Buster`` image was dropped in August 2022 and everyone is expected to
...
building and verifying the images happens in our CI but no unit tests were executed using this image in
the `main` branch.

## Approach to dependencies of Airflow

Airflow has a lot of dependencies - direct and transitive, also Airflow is both - library and application,
therefore our policies on dependencies has to include both - stability of installation of the application,
and also ability to install newer version of dependencies for those users who develop DAGs. We developed
...
needed because of the importance of the dependency as well as the risk it involves to upgrade specific dependencies.
We also upper-bound the dependencies that we know cause problems.

Our constraint mechanism takes care of finding and upgrading all the non-upper bound dependencies
...
Whenever we upper-bound such a dependency, we should always comment on why we are doing it - i.e. we should have
a good reason why the dependency is upper-bound. And we should also mention what is the condition to remove the
binding.
...
known to follow a predictable versioning scheme, and we know that new versions of those are very likely to
bring breaking changes. We commit to regularly reviewing and attempting to upgrade to the newer versions of
the dependencies as they are released, but this is a manual process.

The important dependencies are:

* `SQLAlchemy`: upper-bound to specific MINOR version (SQLAlchemy is known to remove deprecations and
   introduce breaking changes especially since support for different Databases varies and changes at
   various speed (example: SQLAlchemy 1.4 broke MSSQL integration for Airflow)
* `Alembic`: It is important to handle our migrations in a predictable and performant way. It is developed
   together with SQLAlchemy. Our experience with Alembic is that it very stable in the MINOR version
* `Flask`: We are using Flask as the backbone of our web UI and API. We know the major versions of Flask
   are very likely to introduce breaking changes across those so limiting it to a MAJOR version makes sense
* `werkzeug`: The library is known to cause problems in new versions. It is tightly coupled with the Flask
   libraries, and we should update them together
* `celery`: Celery is a crucial component of Airflow as it is used for CeleryExecutor (and similar). Celery
   ...
   we should make sure the Celery Provider minimum Airflow version is updated.
* `kubernetes`: Kubernetes is a crucial component of Airflow as it is used for the KubernetesExecutor
   ...
   we should make sure the Kubernetes Provider minimum Airflow version is updated.
...
have a set of policies implemented for maintaining and releasing community-managed providers as well
...
Those `extras` and `providers` dependencies are maintained in the `provider.yaml` of each provider.

By default, we should not have upper-bound dependencies for providers, however each provider's maintainer
might decide to add additional limits (and justify them with comments).
...
If you would love to have Apache Airflow stickers, t-shirts, etc. then check out
